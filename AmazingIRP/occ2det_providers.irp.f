...
BEGIN_PROVIDER [ integer(bit_kind), gen_dets, (n_int,2,n_det) ]
 ...
 n_int_shift_bit = size_orbital_bucket - leadz(n_int) + 1

 ! Shift in n_int to go to alpha spin part. tmp_det
 ! start with beta spins
 n_int_shift = ibset(0, n_int_shift_bit-1)
 ...
 do i=1,n_det
   ...
   do k=1,n_single_orbital
      ...
      iint = iint + iand(ishft(v,n_int_shift_bit-k),n_int_shift) 
   ...
   do k=1,n_int
      gen_dets(k,1,i) = ior(occ(k,2), tmp_det(k+n_int_shift))
      gen_dets(k,2,i) = ior(occ(k,2), tmp_det(k))
   enddo
